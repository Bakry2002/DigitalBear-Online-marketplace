...
var cashed = global.payload; // we are going to cash our payload so we don't have to make a request to our server every time we want to get our payload
if (!cashed) {
    // if our payload is not cashed then we are going to make a request to our server to get our payload
    cashed = global.payload = {
        client: null,
        promise: null,
    };
}
        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    ...
                    if (cashed.client) {
                        return [2 /*return*/, cashed.client]; // if our payload is cashed then we are going to return our cashed payload
                    }
                    // if we don't have a promise then we are going to make a request to our server to get our payload
                    if (!cashed.promise) {
                        cashed.promise = payload_1.default.init(__assign({ email: {
                ...
                case 1:
                    _d.trys.push([1, 3, , 4]);
                    _c = cashed;
                    return [4 /*yield*/, cashed.promise];
                case 2:
                    _c.client = _d.sent(); // we are going to wait for our promise to resolve and then we are going to cash our payload
                    return [3 /*break*/, 4];
                case 3:
                    error_1 = _d.sent();
                    cashed.promise = null; // if we get an error then we are going to set our promise to null so we can try again
                    throw error_1;
                case 4: return [2 /*return*/, cashed.client];
            }
        });
    });
};
